...
if config.STRATEGY == "bulldog":
    ...
    from strategy_probability import (
        ProbabilityStrategy as Strategy,
        TradeSignal,
        Signal,
    )
...
class TradingBot:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Mode: {'PAPER TRADING' if self.paper_trading else '[LIVE] LIVE TRADING'}"
        )
    ...
    def _load_state(self):
        ...
        try:
            ...
            if self.paper_position:
                logger.info(
                    f"  Open position: {self.paper_position['side'].upper()} @ {self.paper_position['entry_price']:.2f}"
                )
        except Exception as e:
    ...
    def connect_exchange(self) -> bool:
        ...
        try:
            ...
            if not config.API_KEY or not config.API_SECRET:
                logger.error(
                    "API credentials not set. Set POLONIEX_API_KEY and POLONIEX_API_SECRET"
                )
                return False
            ...
            logger.info(
                f"Connected to Poloniex. USDT Balance: {balance.get('USDT', {}).get('free', 0):.2f}"
            )
            return True
    ...
    def fetch_ohlcv(self, limit: Optional[int] = None) -> Optional[pd.DataFrame]:
        ...
        for attempt in range(3):
            try:
                ...
                wait = 10 * (attempt + 1)
                logger.warning(
                    f"Network error (attempt {attempt + 1}/3), retrying in {wait}s: {e}"
                )
                time.sleep(wait)
            except Exception as e:
    ...
    def _paper_trade(self, signal: TradeSignal, side: str) -> bool:
        """Execute paper trade, charging the entry taker fee upfront"""
        entry_notional = signal.entry_price * signal.position_size
        entry_fee = entry_notional * config.PAPER_TAKER_FEE
        ...
        # Entry fee hits the balance immediately, exactly like a real fill
        self.paper_balance -= entry_fee
        self.paper_pnl -= entry_fee
        ...
        logger.info(f"  Notional: ${entry_notional:.2f} | Entry fee: ${entry_fee:.2f}")
    ...
    def _live_trade(self, signal: TradeSignal, side: str) -> bool:
        ...
        try:
            ...
            order = self.exchange.create_market_order(
                config.SYMBOL, side, signal.position_size
            )
    ...
    def check_paper_exit(
        self, current_price: float, candle_high: float = 0.0, candle_low: float = 0.0
    ):
    ...
    def _close_paper_position(self, pnl: float, reason: str):
        """Close paper position, charging exit fee + funding, and update balance.
        ...
        if hasattr(self.strategy, "record_trade_result"):
            self.strategy.record_trade_result(won=(net_pnl > 0))
        ...
        logger.info(f"  Gross PnL: ${pnl:.2f}")
        logger.info(
            f"  Costs: ${total_costs:.2f} "
            f"(entry ${pos.get('entry_fee', 0.0):.2f} + exit ${exit_fee:.2f} "
            f"+ funding ${funding:.2f} over {pos.get('candles_held', 0)} candles)"
        )
    ...
    def _regime_gate(self, signal) -> Optional[str]:
        ...
        if (
            getattr(config, "BLOCK_RANGING_REGIME", False)
            and snap.market_regime == "RANGING"
        ):
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                now = datetime.now(timezone.utc)
                should_scout = (
                    self.last_scout_time is None
                    or (now - self.last_scout_time) >= self.scout_interval
                )
                ...
                if self.paper_trading:
                    ...
                    # Funding accrues per candle held (loop runs once per candle)
                    if self.paper_position is not None:
                        self.paper_position["candles_held"] = (
                            self.paper_position.get("candles_held", 0) + 1
                        )
                        self._save_state()
                ...
                if today != self.daily_pnl_date:
                    logger.info(
                        f"[DAILY RESET] New trading day ({today}). Daily PnL reset from ${self.daily_pnl:.2f} to $0.00"
                    )
                    self.daily_pnl = 0.0
                    self.daily_pnl_date = today
                ...
                if balance > 0:
                    ...
                    if daily_loss_pct >= self.max_daily_loss:
                        logger.warning(
                            f"[!] Daily loss limit reached ({daily_loss_pct:.1%}). Trading paused until next day."
                        )
                ...
                if self.has_open_position():
                    ...
                    and self.current_position.get("sl_order_id")
                ):
                ...
                if signal:
                    ...
                    if block_reason:
                        logger.info(
                            f"[REGIME BLOCK] {signal.signal.value} entry filtered — {block_reason}"
                        )
                        signal = None
